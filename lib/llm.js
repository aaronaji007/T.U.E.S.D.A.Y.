...
async function warmModel() {
  const provider = getProvider();
  if (provider === 'openai' || provider === 'gemini') return { ok: true };
...
async function rawChat({ messages, tools, format, temperature }) {
  ...
  if (provider === 'openai' || provider === 'gemini') {
    let apiKey = process.env.OPENAI_API_KEY;
    let baseUrl = process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1';
    let targetModel = process.env.OPENAI_MODEL || 'gpt-4o-mini';

    if (provider === 'gemini') {
      apiKey = process.env.GEMINI_API_KEY;
      baseUrl = 'https://generativelanguage.googleapis.com/v1beta/openai';
      targetModel = process.env.GEMINI_MODEL || 'gemini-1.5-flash';
    }
